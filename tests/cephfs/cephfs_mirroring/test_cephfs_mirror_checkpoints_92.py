    ...
    try:
        ...
        try:
            ...
            for attempt in range(20):
                ...
                log.info(
                    f"[S2 Poll {attempt}] state={ps.get('state')}, last_synced={last}"
                )
            ...
            for ckpt in ckpt_list_after:
                if ckpt.get("snap_name") == "snap_now":
                    ...
                    break
            if now_ckpt_after:
                log.info(f"Checkpoint still references snap_now: {now_ckpt_after}")
            ...
            log.info(f"Checkpoint 'now' may not be supported yet: {e}")
