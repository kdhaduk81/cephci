...
def run(ceph_cluster, **kw):
    ...
    Covers functional tests:
     ...
     7. MGR module bounce preserves checkpoint state
    ...
    try:
        ...
        try:
            ...
            for attempt in range(20):
                ...
                log.info(
                    ...
                    attempt, ps.get("state"), last,
                )
                if ps.get("state") == "idle" and last == "snap_after_now":
                    break
            else:
                raise CommandFailed(
                    "S2: snap_after_now did not reach idle within 300s"
                )
            ...
            if now_ckpt_after:
                log.info(
                    "Checkpoint still references snap_now: %s", now_ckpt_after
                )
                log.info("Scenario 2 PASSED: checkpoint now is pinned to snap_now")
            else:
                log.warning(
                    "Checkpoint for snap_now not found after new snap created"
                )
        ...
        # ============================================================
        # Scenario 7: MGR module bounce preserves checkpoint state
        # ============================================================
        log.info("=" * 60)
        log.info("Scenario 7: MGR module bounce preserves checkpoint state")
        log.info("=" * 60)

        source_clients[0].exec_command(
            sudo=True,
            cmd=f"dd if=/dev/urandom of={mount_path}file_mgr_bounce bs=1M count=100",
        )
        source_clients[0].exec_command(
            sudo=True, cmd=f"mkdir {mount_path}.snap/snap_mgr_bounce"
        )
        fs_mirroring_utils.validate_snapshot_sync_status(
            cephfs_mirror_node[0], source_fs, "snap_mgr_bounce",
            fsid, asok_file, filesystem_id, peer_uuid,
        )

        checkpoint_add(
            source_clients[0], source_fs, subvol_path1, "snap_mgr_bounce"
        )
        time.sleep(10)
        ckpt_before_bounce = checkpoint_ls(
            source_clients[0], source_fs, subvol_path1
        )
        log.info(f"S7: Checkpoints before MGR bounce: {ckpt_before_bounce}")

        before_map = {c.get("snap_name"): c for c in ckpt_before_bounce}
        if "snap_mgr_bounce" not in before_map:
            raise CommandFailed(
                "S7 FAILED: snap_mgr_bounce checkpoint not found before bounce"
            )
        ...
        ckpt_after_bounce = checkpoint_ls(
            source_clients[0], source_fs, subvol_path1
        )
        log.info(f"S7: Checkpoints after MGR bounce: {ckpt_after_bounce}")

        after_map = {c.get("snap_name"): c for c in ckpt_after_bounce}
        if "snap_mgr_bounce" not in after_map:
            raise CommandFailed(
                "S7 FAILED: snap_mgr_bounce checkpoint lost after MGR bounce"
            )

        if before_map["snap_mgr_bounce"].get("status") != after_map["snap_mgr_bounce"].get("status"):
            raise CommandFailed(
                f"S7 FAILED: status changed after bounce: "
                f"{before_map['snap_mgr_bounce'].get('status')} -> "
                f"{after_map['snap_mgr_bounce'].get('status')}"
            )

        log.info("S7 PASSED: Checkpoint state preserved across MGR module bounce")
