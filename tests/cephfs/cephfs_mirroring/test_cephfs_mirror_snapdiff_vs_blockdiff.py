...
def run(ceph_cluster, **kw):
    ...
    try:
        ...
        # Record log position before sync so we only grep new lines
        ...
        # Validate snapdiff: small file should have num_blocks=1, block covers entire file
        ...
        if small_blocks is not None:
            ...
            if len(small_blocks) == 1:
                offset, length = small_blocks[0]
                expected_len = 4 * 1024 * 1024  # 4 MB
                if offset == 0 and length == expected_len:
                    log.info(
                        "PASS: %s - snapdiff copied entire file as one block [0~%d]",
                        small_modify_file,
                        expected_len,
                    )
                else:
                    log.warning(
                        "WARN: %s - unexpected block [%d~%d], expected [0~%d]",
                        small_modify_file,
                        offset,
                        length,
                        expected_len,
                ...
                log.warning(
                    "WARN: %s - expected 1 block, got %d",
                    small_modify_file,
                    len(small_blocks),
                )
        else:
            log.warning(
                "WARN: Could not parse copy_to_remote blocks for %s",
                small_modify_file,
            )

        # Validate blockdiff: large file should sync only the modified 1MB region
        ...
        if large_blocks is not None:
            ...
            if len(large_blocks) == 1:
                offset, length = large_blocks[0]
                expected_offset = large_modify_offset * 1024 * 1024
                expected_len = large_modify_count * 1024 * 1024
                if offset == expected_offset and length == expected_len:
                    log.info(
                        "PASS: %s - blockdiff synced only modified block [%d~%d]",
                        large_modify_file,
                        expected_offset,
                        expected_len,
                    )
                ...
                log.warning(
                    "WARN: %s - expected 1 block for single-region modify, got %d",
                    large_modify_file,
                    len(large_blocks),
                )
        else:
        ...
        if multi_blocks is not None:
            log.info(
                "Multi-region blockdiff: num_blocks=%d, blocks=%s",
                len(multi_blocks),
                multi_blocks,
            )
            expected_b1 = (region1_offset * 1024 * 1024, 1024 * 1024)
            expected_b2 = (region2_offset * 1024 * 1024, 1024 * 1024)
            if len(multi_blocks) == 2:
                if multi_blocks[0] == expected_b1 and multi_blocks[1] == expected_b2:
                    log.info(
                        "PASS: %s - blockdiff synced exactly 2 modified blocks "
                        "[%d~%d] and [%d~%d]",
                        large_modify_file,
                        expected_b1[0],
                        expected_b1[1],
                    ...
                    log.warning(
                        "WARN: %s - blocks %s don't match expected %s, %s",
                        large_modify_file,
                        multi_blocks,
                        expected_b1,
                        expected_b2,
                ...
                log.warning(
                    "WARN: %s - expected 2 blocks for dual-region modify, got %d",
                    large_modify_file,
                    len(multi_blocks),
                )
        else:
            log.warning(
                "WARN: Could not parse copy_to_remote blocks for %s (snap3)",
                large_modify_file,
            )
