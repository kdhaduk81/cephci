            ...
            try:
                ...
                for entry in data.get("cephfs_mirror_directory", []):
                    ...
                    if subvol_path1.rstrip("/") in dir_label:
                        ...
                        log.info(
                            f"[DirState Poll {poll_i}] dir_state={dir_state}, "
                            f"snap_id={counters.get('current_snap_id', 0)}, "
                            f"sync_bytes={counters.get('current_sync_bytes', 0)}, "
                            f"total_bytes={counters.get('current_total_bytes', 0)}, "
                            f"sync_pct={counters.get('current_sync_bytes_percent', 0)}, "
                            f"sync_files={counters.get('current_sync_files', 0)}, "
                            f"crawl_state={counters.get('crawl_state', 0)}, "
                            f"sync_mode={counters.get('current_sync_mode', 0)}, "
                            f"snaps_synced={counters.get('snaps_synced', 0)}"
                        )
                        if dir_state == 1 and not syncing_counters_captured:
                            syncing_counters_captured = True
                            log.info(
                                f"[DirState] Captured syncing counters: "
                                f"read_bps={counters.get('current_read_bps', 0)}, "
                                f"write_bps={counters.get('current_write_bps', 0)}, "
                                f"eta_valid={counters.get('current_eta_valid', 0)}, "
                                f"eta_secs={counters.get('current_eta_seconds', 0)}, "
                                f"datasync_wait={counters.get('datasync_wait_state', 0)}"
                            )
                        if dir_state == 0 and counters.get("snaps_synced", 0) >= 1:
                            log.info(
                                f"[DirState Poll {poll_i}] snap synced, "
                ...
                if not found_entry:
                    log.info(
                        f"[DirState Poll {poll_i}] no matching entry for "
                        f"{subvol_path1.rstrip('/')}"
                    )
            except Exception as e:
