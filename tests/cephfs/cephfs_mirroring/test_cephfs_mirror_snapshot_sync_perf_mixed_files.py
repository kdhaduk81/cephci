...
log_dir = f"{log_base_dir}/MixedFileSyncPerf_Results/"
os.makedirs(log_dir, exist_ok=True)
log.info(f"Log Dir : {log_dir}")

GENERATE_SCRIPT = "generate_mixed_dataset.py"
GENERATE_SCRIPT_SRC = (
    f"tests/cephfs/cephfs_mirroring/snapdiff_scripts/{GENERATE_SCRIPT}"
)
GENERATE_SCRIPT_DST = f"/root/{GENERATE_SCRIPT}"

DEFAULT_THREAD_COUNTS = [1, 3, 6, 9, 12]

DEFAULT_NUM_SMALL_FILES = 10000
DEFAULT_MIN_SMALL_FILE_SIZE_KB = 2
DEFAULT_MAX_SMALL_FILE_SIZE_KB = 256
DEFAULT_NUM_SMALL_DIRS = 100

DEFAULT_NUM_LARGE_FILES = 10
DEFAULT_LARGE_FILE_SIZE_MB = 1024  # 1 GiB each
DEFAULT_NUM_LARGE_DIRS = 10

DEFAULT_SUBVOL_SIZE_BYTES = "107374182400"  # 100 GiB
...
def run(ceph_cluster, **kw):
    """
    Snapshot Sync Performance Test -- Mixed Dataset (CephFS Mirroring)

    Measures how the cephfs_mirror_max_datasync_threads tunable affects
    snapshot synchronisation throughput for a mixed workload of many small
    files spread across directories plus a few large files spread across
    directories, tested on kernel, fuse and nfs mount types.

    Default dataset per subvolume:
        10,000 small files (2KB-256KB) across 100 directories
      + 10 x 1GiB large files across 10 directories
      = ~11.2 GiB total (10,010 files)
    ...
    For each configured thread count the test:
        ...
        3. Generates the mixed dataset sequentially into those directories.
    ...
    Config keys (all optional, sane defaults provided):
        result_file            : str        -- CSV filename
        thread_counts          : list[int]  -- default [1,3,6,9,12]
        num_small_files        : int        -- small files total, default 10000
        min_small_file_size_kb : int        -- min small file KiB, default 2
        max_small_file_size_kb : int        -- max small file KiB, default 256
        num_small_dirs         : int        -- dirs for small files, default 100
        num_large_files        : int        -- large files total, default 10
        large_file_size_mb     : int        -- each large file MB, default 1024
        num_large_dirs         : int        -- dirs for large files, default 10
        subvol_size            : str        -- subvolume quota bytes, default 100 GiB
        file_gen_timeout       : int        -- seconds for dataset gen, default 10800
        sync_poll_tries        : int        -- max poll attempts, default 360
    ...
    fs_util_ceph1 = None
    peer_uuid = None
    subvol_group = "subvolgroup_perf_mix"
    subvolumegroup_list = []
    created_subvols = {}
    subvol_paths = {}
    mirroring_paths = []
    ...
    try:
        ...
        num_small_files = config.get("num_small_files", DEFAULT_NUM_SMALL_FILES)
        min_kb = config.get("min_small_file_size_kb", DEFAULT_MIN_SMALL_FILE_SIZE_KB)
        max_kb = config.get("max_small_file_size_kb", DEFAULT_MAX_SMALL_FILE_SIZE_KB)
        num_small_dirs = config.get("num_small_dirs", DEFAULT_NUM_SMALL_DIRS)
        num_large_files = config.get("num_large_files", DEFAULT_NUM_LARGE_FILES)
        large_file_size_mb = config.get(
            "large_file_size_mb", DEFAULT_LARGE_FILE_SIZE_MB
        )
        num_large_dirs = config.get("num_large_dirs", DEFAULT_NUM_LARGE_DIRS)
        subvol_size = config.get("subvol_size", DEFAULT_SUBVOL_SIZE_BYTES)
        file_gen_timeout = config.get("file_gen_timeout", 10800)
        sync_poll_tries = config.get("sync_poll_tries", 360)
        sync_poll_delay = config.get("sync_poll_delay", 15)

        total_files = num_small_files + num_large_files
        avg_small_kb = (min_kb + max_kb) / 2
        est_total_mb = (num_small_files * avg_small_kb / 1024) + (
            num_large_files * large_file_size_mb
        )

        log.info(
            "Test parameters: thread_counts=%s, small_files=%d (%d-%dKB in %d dirs), "
            "large_files=%d (%dMB in %d dirs), est_total=%.1fMB per subvolume",
            thread_counts,
            num_small_files,
            min_kb,
            max_kb,
        ...
        source_clients[0].upload_file(
            sudo=True, src=GENERATE_SCRIPT_SRC, dst=GENERATE_SCRIPT_DST
        )
        ...
        result_file = config.get("result_file", "mixed_file_sync_perf.csv")
        ...
        for thread_count in thread_counts:
            ...
            gen_start = time.time()
            for mtype in MOUNT_TYPES:
                log.info("Generating mixed dataset on %s...", mtype)
                _generate_mixed_on_client(
                    source_clients[0],
                    io_dir_paths[mtype],
                    num_small_files,
                    min_kb,
                ...
                log.info("Mixed dataset generation completed for %s", mtype)

            gen_duration = time.time() - gen_start
            log.info("All dataset generation completed in %.1f seconds", gen_duration)
            ...
            iter_snapshots = {}
            for mtype in MOUNT_TYPES:
                snap_name = f"snap_mperf_t{thread_count}_{mtype[0]}"
            ...
            for mtype in MOUNT_TYPES:
                ...
                if daemon_sync_duration is not None and daemon_sync_duration > 0:
                    files_per_sec = total_files / daemon_sync_duration
...
def _generate_mixed_on_client(
    ...
    """Run the mixed dataset generation script on the remote client."""
    client.exec_command(
        sudo=True,
        timeout=timeout,
        cmd=(
            f"python3 {GENERATE_SCRIPT_DST} "
            f"{io_dir} "
            f"{num_small_files} {min_kb} {max_kb} {num_small_dirs} "
            f"{num_large_files} {large_file_size_mb} {num_large_dirs}"
        ),
        long_running=True,
    )
...
def _initialize_csv(csv_file, ceph_version, mirror_node, config):
    ...
    with open(csv_file, mode="w", newline="") as f:
        ...
        writer.writerow(
            [
                f"Small files: {config.get('num_small_files', DEFAULT_NUM_SMALL_FILES)} "
                f"({config.get('min_small_file_size_kb', DEFAULT_MIN_SMALL_FILE_SIZE_KB)}-"
                f"{config.get('max_small_file_size_kb', DEFAULT_MAX_SMALL_FILE_SIZE_KB)} KB "
                f"in {config.get('num_small_dirs', DEFAULT_NUM_SMALL_DIRS)} dirs), "
                f"Large files: {config.get('num_large_files', DEFAULT_NUM_LARGE_FILES)} x "
                f"{config.get('large_file_size_mb', DEFAULT_LARGE_FILE_SIZE_MB)} MB "
                f"in {config.get('num_large_dirs', DEFAULT_NUM_LARGE_DIRS)} dirs"
            ]
        )
        writer.writerow([])
        writer.writerow(
