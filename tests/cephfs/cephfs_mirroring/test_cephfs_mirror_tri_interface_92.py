...
def collect_tri_interface(
    ...
    try:
        ...
        log.info(
            f"{prefix} Asok: state={result['asok'].get('state')}, "
            f"snaps_synced={result['asok'].get('snaps_synced')}, "
            f"current_syncing_snap={result['asok'].get('current_syncing_snap', {}).get('name') if result['asok'].get('current_syncing_snap') else None}"
        )
    ...
    try:
        ...
        log.info(
            f"{prefix} MGR: state={result['mgr'].get('state')}, "
            f"snaps_synced={result['mgr'].get('snaps_synced')}, "
            f"last_synced={result['mgr'].get('last_synced_snap', {}).get('name')}"
        )
    except Exception as e:
        log.warning(f"{prefix} MGR query failed: {e}")
    ...
    try:
        ...
        for entry in data.get("cephfs_mirror_directory", []):
            if path_key in entry.get("labels", {}).get("directory", ""):
                ...
                log.info(f"{prefix} Perf: {result['perf']}")
                break
        if not result["perf"]:
            log.info(f"{prefix} Perf: no matching entry for {path_key}")
    except Exception as e:
        log.warning(f"{prefix} Perf counter query failed: {e}")
