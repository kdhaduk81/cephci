...
log_dir = f"{log_base_dir}/SmallFileSyncPerf_Results/"
os.makedirs(log_dir, exist_ok=True)
log.info(f"Log Dir : {log_dir}")

GENERATE_SCRIPT = "generate_small_files_random_sizes.py"
GENERATE_SCRIPT_SRC = (
...
DEFAULT_NUM_FILES = 10000
DEFAULT_MIN_FILE_SIZE_KB = 2
DEFAULT_MAX_FILE_SIZE_KB = 256
DEFAULT_SUBVOL_SIZE_BYTES = "34359738368"  # 32 GiB
...
def run(ceph_cluster, **kw):
    """
    Snapshot Sync Performance Test -- Small Files (CephFS Mirroring)

    Measures how the cephfs_mirror_max_datasync_threads tunable affects
    snapshot synchronisation throughput for a workload of many small files
    ...
    For each configured thread count the test:
        ...
        3. Generates N small files (random 2KB-256KB) into those directories.
    ...
    Config keys (all optional, sane defaults provided):
        result_file      : str        -- CSV filename, default small_file_sync_perf.csv
        thread_counts    : list[int]  -- thread counts to test, default [1,3,6,9,12]
        num_files        : int        -- files per subvolume, default 10000
        min_file_size_kb : int        -- minimum random file size in KiB, default 2
        max_file_size_kb : int        -- maximum random file size in KiB, default 256
        subvol_size      : str        -- subvolume quota in bytes, default 32 GiB
        file_gen_timeout : int        -- seconds for file generation cmd, default 3600
        sync_poll_tries  : int        -- max poll attempts for sync, default 120
    ...
    try:
        ...
        result_file = config.get("result_file", "small_file_sync_perf.csv")
        ...
        for thread_count in thread_counts:
            ...
            # Re-resolve ASOK after daemon restart (may move to a different node)
            asok_file = fs_mirroring_utils.get_asok_file(
                cephfs_mirror_node, fsid, daemon_name
            )
            ...
            # ---- 3. Generate small files on all 3 mount types ----
            log.info(
                "Generating %d files (%d-%d KB) on kernel/fuse/nfs",
                num_files,
                min_kb,
                max_kb,
            )
            ...
            iter_snapshots = {}
            for mtype in MOUNT_TYPES:
                snap_name = f"snap_perf_t{thread_count}_{mtype[0]}"
...
def _generate_files_on_client(client, io_dir, num_files, min_kb, max_kb, timeout):
    """Run the file generation script on the remote client."""
    client.exec_command(
        sudo=True,
        timeout=timeout,
        cmd=(
            f"python3 {GENERATE_SCRIPT_DST} " f"{io_dir} {num_files} {min_kb} {max_kb}"
        ),
        long_running=True,
    )
    ...
    with open(csv_file, mode="w", newline="") as f:
        ...
        writer.writerow(
            [
                ...
                f"Size range: {config.get('min_file_size_kb', DEFAULT_MIN_FILE_SIZE_KB)}-"
                f"{config.get('max_file_size_kb', DEFAULT_MAX_FILE_SIZE_KB)} KB"
            ]
        )
        writer.writerow([])
        writer.writerow(
